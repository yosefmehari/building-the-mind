...
const COPY: Record<Locale, { courses: string; fullStack: string; english: string; studentSignIn: string; dashboard: string; search: string; notifications: string; close: string; caughtUp: string; updates: string; allRead: string; unread: string }> = {
  en: { courses: "Courses", fullStack: "Full Stack", english: "English", studentSignIn: "Student sign in", dashboard: "Dashboard", search: "Search", notifications: "Notifications", close: "Close", caughtUp: "You are all caught up.", updates: "New course updates will appear here.", allRead: "All notifications are read", unread: "Unread notification" },
  ti: { courses: "ትምህርቲታት", fullStack: "Full Stack", english: "እንግሊዝኛ", studentSignIn: "ተማሃራይ እቶ", dashboard: "ዳሽቦርድ", search: "ድለ", notifications: "መልእኽትታት", close: "ዕጸው", caughtUp: "ኩሉ ሓበሬታ ኣንቢብካዮ ኢኻ።", updates: "ሓደሽቲ ዜናታት ትምህርቲ ኣብዚ ክርአዩ እዮም።", allRead: "ኩሎም መልእኽትታት ተነቢቦም", unread: "ዘይተነበበ መልእኽቲ" },
  am: { courses: "ኮርሶች", fullStack: "Full Stack", english: "እንግሊዝኛ", studentSignIn: "የተማሪ መግቢያ", dashboard: "ዳሽቦርድ", search: "ፈልግ", notifications: "ማሳወቂያዎች", close: "ዝጋ", caughtUp: "ሁሉንም አይተዋል።", updates: "አዲስ የኮርስ ዝመናዎች እዚህ ይታያሉ።", allRead: "ሁሉም ማሳወቂያዎች ተነብበዋል", unread: "ያልተነበበ ማሳወቂያ" },
};
...
export function Navbar() {
  ...
  return (
    <nav className={cn("fixed top-0 left-0 right-0 z-50 transition-all duration-300", scrolled ? "bg-slate-950/90 backdrop-blur-xl border-b border-slate-800/80 shadow-2xl" : "bg-transparent")}>
      ...
      {isMenuOpen && <div className="md:hidden bg-slate-950/95 backdrop-blur-xl border-t border-slate-800 px-4 py-4 space-y-1">{NAV_LINKS.map((link) => <Link key={link.href} href={link.href} onClick={() => setIsMenuOpen(false)} className="flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-sm font-medium text-slate-300 hover:text-white hover:bg-slate-800">{link.icon}{copy[link.key]}{link.badge && <Badge variant="indigo" size="sm">{link.badge}</Badge>}</Link>)}<div className="pt-3 border-t border-slate-800 flex gap-2">{role === "admin" ? <><Link href="/admin" onClick={() => setIsMenuOpen(false)} className="flex-1 rounded-lg border border-indigo-500/40 bg-indigo-500/10 px-2 py-2 text-center text-xs font-medium text-indigo-300">Admin Dashboard</Link><Link href="/dashboard" onClick={() => setIsMenuOpen(false)} className="flex-1 rounded-lg border border-slate-800 px-2 py-2 text-center text-xs font-medium text-slate-300">{copy.dashboard}</Link></> : <><Link href="/login" onClick={() => setIsMenuOpen(false)} className="flex-1 rounded-lg border border-indigo-500/40 bg-indigo-500/10 px-2 py-2 text-center text-xs font-medium text-indigo-300">{copy.studentSignIn}</Link><Link href="/dashboard" onClick={() => setIsMenuOpen(false)} className="flex-1 rounded-lg border border-slate-800 px-2 py-2 text-center text-xs font-medium text-slate-300">{copy.dashboard}</Link></>}</div><div className="flex items-center justify-between gap-3 border-t border-slate-800 pt-3"><span className="text-xs font-medium text-slate-500">Currency</span><div className="flex items-center gap-2"><ThemeToggle /><CurrencySwitcher onCurrencyChange={() => { router.refresh(); setIsMenuOpen(false); }} /></div></div><div className="flex gap-2">{LOCALES.map((item) => <button key={item.code} onClick={() => void selectLocale(item.code)} className={cn("flex-1 px-2 py-1.5 rounded-lg text-xs font-medium border transition", locale === item.code ? "border-indigo-500/50 bg-indigo-500/15 text-indigo-300" : "border-slate-800 text-slate-400 hover:text-white")}>{item.flag} {item.label}</button>)}</div></div>}
    </nav>
  );
}
