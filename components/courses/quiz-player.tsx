...
export function QuizPlayer({ quiz, signedIn }: { quiz: Quiz; signedIn: boolean }) {
  ...
  return <section className="rounded-2xl border border-rose-500/20 bg-slate-900/60 p-6"><div className="flex items-start gap-3"><ClipboardCheck className="mt-0.5 h-5 w-5 shrink-0 text-rose-300" /><div><h2 className="font-bold text-white">{quiz.title}</h2>{quiz.description && <p className="mt-1 text-sm text-slate-400">{quiz.description}</p>}</div></div>{state?.score !== undefined ? <div className="mt-6 rounded-xl border border-emerald-500/20 bg-emerald-500/10 p-5"><div className="flex items-center gap-2 text-emerald-300"><CheckCircle2 className="h-5 w-5" /><span className="font-semibold">Quiz submitted</span></div><p className="mt-2 text-2xl font-black text-white">{state.score} / {state.total}</p><p className="mt-1 text-sm text-slate-400">Your result has been saved.</p></div> : <form action={action} className="mt-6 space-y-6"><input type="hidden" name="quizId" value={quiz.id} />{quiz.questions.map((question, index) => <fieldset key={question.id} className="space-y-3"><legend className="text-sm font-medium text-slate-200">{index + 1}. {question.question}</legend><div className="grid gap-2 sm:grid-cols-2">{question.answers.map((answer) => <label key={answer.id} className="flex cursor-pointer items-center gap-3 rounded-xl border border-slate-800 bg-slate-950/40 px-4 py-3 text-sm text-slate-300 transition hover:border-rose-500/40 has-checked:border-rose-500/50 has-checked:bg-rose-500/10"><input type="radio" name={`question-${question.id}`} value={answer.id} required className="h-4 w-4 accent-rose-500" />{answer.answer}</label>)}</div></fieldset>)}{state?.error && <p role="alert" className="text-sm text-rose-300">{state.error}</p>}<Button type="submit" disabled={pending} isLoading={pending}>Submit quiz</Button></form>}</section>;
}
