    ...
    <main className="flex-1 px-4 py-10">
      <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div className="min-w-0 space-y-7">
          ...
          <section className="rounded-2xl border border-slate-800 bg-slate-900/60 p-6"><div className="flex items-center justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.18em] text-slate-500">Course progress</p><p className="mt-1 text-sm text-slate-300">{completedCount} of {allLessons.length} lessons completed</p></div><span className="text-lg font-bold text-indigo-300">{progress}%</span></div><div className="mt-4 h-2 overflow-hidden rounded-full bg-slate-800"><div className="h-full rounded-full bg-indigo-500 transition-all" style={{ width: `${progress}%` }} /></div><div className="mt-4"><LessonProgress lessonId={lesson.id.toString()} courseSlug={course.slug} lessonSlug={lesson.slug} completed={currentLessonCompleted} signedIn={Boolean(session)} requiredExam={lesson.quizzes.some((quiz) => quiz.required && quiz.questions.length > 0)} /></div></section>
          ...
          {lesson.quizzes.filter((quiz) => quiz.questions.length > 0).map((quiz) => <QuizPlayer key={quiz.id.toString()} quiz={{ id: quiz.id.toString(), title: quiz.title, description: quiz.description, questions: quiz.questions.map((question) => ({ id: question.id.toString(), question: question.question, answers: question.answers.map((answer) => ({ id: answer.id.toString(), answer: answer.answer })) })) }} requiredForCompletion={quiz.required} completionTarget={{ lessonId: lesson.id.toString(), courseSlug: course.slug, lessonSlug: lesson.slug }} signedIn={Boolean(session)} />)}
