      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60">
            ...
            {lessons.length > 0 ? <div className="divide-y divide-slate-800/80">{lessons.map((lesson) => <div key={lesson.id.toString()} className="flex items-center gap-4 px-6 py-4"><span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-emerald-500/10 text-xs font-bold text-emerald-300">{lesson.position + 1}</span><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="truncate text-sm font-medium text-white">{lesson.title}</h3><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${lesson.is_free ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border border-amber-500/20"}`}>{lesson.is_free ? "Free" : formatPrice(Number(lesson.price ?? 0), lesson.currency)}</span></div><p className="mt-1 truncate text-xs text-slate-500">{lesson.modules.courses.title} / {lesson.modules.title} · {lesson._count.videos} videos · {lesson._count.lesson_files} files</p></div><span className={`rounded-full px-2.5 py-1 text-xs font-medium ${lesson.published ? "bg-emerald-500/10 text-emerald-300" : "bg-slate-800 text-slate-400"}`}>{lesson.published ? "Published" : "Draft"}</span></div>)}</div> : <div className="px-6 py-14 text-center"><Plus className="mx-auto h-7 w-7 text-slate-600" /><p className="mt-3 text-sm text-slate-500">No lessons have been created yet.</p></div>}
          </section>
