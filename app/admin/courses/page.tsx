      ...
      <div className="mx-auto max-w-6xl space-y-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
          <section className="overflow-hidden rounded-2xl border border-slate-800 bg-slate-900/60">
            ...
            {courses.length > 0 ? <div className="divide-y divide-slate-800/80">{courses.map((course) => <div key={course.id.toString()} className="flex flex-col gap-3 px-6 py-5 sm:flex-row sm:items-center"><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><h3 className="truncate font-medium text-white">{course.title}</h3><span className={`rounded-full px-2 py-0.5 text-[11px] font-semibold ${course.is_free ? "bg-emerald-500/10 text-emerald-400 border border-emerald-500/20" : "bg-amber-500/10 text-amber-400 border border-amber-500/20"}`}>{course.is_free ? "Free" : formatPrice(Number(course.price ?? 0), course.currency)}</span></div><p className="mt-1 text-xs text-slate-500">/{course.slug} · {course._count.modules} modules · {course._count.enrollments} students</p></div><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-medium ${course.published ? "bg-emerald-500/10 text-emerald-300" : "bg-slate-800 text-slate-400"}`}>{course.published ? "Published" : "Draft"}</span><DeleteButton action={deleteCourse} field="courseId" id={course.id.toString()} label={course.title} /></div>)}</div> : <div className="px-6 py-14 text-center"><Plus className="mx-auto h-7 w-7 text-slate-600" /><p className="mt-3 text-sm text-slate-500">Your catalog is empty.</p></div>}
          </section>
